...
def detect_vehicles(video_path):
    ...
    vehicle_cascade = cv2.CascadeClassifier('cars.xml')  # Ensure 'cars.xml' is in the same directory or provide full path

    # Check if the classifier loaded properly
    if vehicle_cascade.empty():
        print("Error: Could not load 'cars.xml'. Make sure the file path is correct.")
        return
